...
class ModelCloneHelper(object):
    ...
    def _clone_prepare(self, duplicate, exclude=None):
        # Setting pk to None tricks Django into not trying to overwrite the old object (we will force the INSERT later)
    ...
    def _clone_attrs(self, duplicate, attrs, exclude=None):
        """
        Will apply the ``attrs`` (a ``dict``) to the given instance. That makes it possible to exclude fields from cloning.
        """
        if attrs:
        ...
        for field in self.instance._meta.many_to_many:
            ...
            if field.rel.through and not field.rel.through._meta.auto_created:
                ...
                if hasattr(field.rel.through, 'clone'):
                    for m2m_obj in field.rel.through._default_manager.filter(**{field.m2m_field_name(): self.instance}):
                        m2m_obj.clone(attrs={field.m2m_field_name(): duplicate})
                else:
                    for m2m_obj in field.rel.through._default_manager.filter(**{field.m2m_field_name(): self.instance}):
                        m2m_clone_helper = ModelCloneHelper(m2m_obj)  # TODO: Allow switching to different helper?
                        m2m_clone_helper.clone(attrs={field.m2m_field_name(): duplicate})
    ...
    def _clone_copy_reverse_m2m(self, duplicate, exclude=None):
        exclude = exclude or []
        for relation in self.instance._meta.get_all_related_many_to_many_objects():
            # handle m2m using through
            if relation.field.rel.through and not relation.field.rel.through._meta.auto_created:
                ...
                if hasattr(relation.field.rel.through, 'clone'):
                    for m2m_obj in relation.field.rel.through._default_manager.filter(**{relation.field.m2m_reverse_field_name(): self.instance}):
                        m2m_obj.clone(attrs={relation.field.m2m_reverse_field_name(): duplicate})
                else:
                    for m2m_obj in relation.field.rel.through._default_manager.filter(**{relation.field.m2m_reverse_field_name(): self.instance}):
                        m2m_clone_helper = ModelCloneHelper(m2m_obj)  # TODO: Allow switching to different helper?
                        m2m_clone_helper.clone(attrs={relation.field.m2m_reverse_field_name(): duplicate})
            # normal m2m, this is easy
            else:
                # Skip this field.
                if relation.field.rel.related_name in exclude:
                    continue
                objs = getattr(self.instance, relation.field.rel.related_name).all()
    ...
    def clone(self, attrs=None, commit=True, m2m_clone_reverse=True, exclude=None):
        ...
        duplicate = getattr(self.instance, '_clone_copy', self._clone_copy)()
        getattr(self.instance, '_clone_prepare', self._clone_prepare)(duplicate, exclude=exclude)
        getattr(self.instance, '_clone_attrs', self._clone_attrs)(duplicate, attrs, exclude=exclude)

        def clone_m2m(clone_reverse=m2m_clone_reverse):
            getattr(self.instance, '_clone_copy_m2m', self._clone_copy_m2m)(duplicate, exclude=exclude)
            if clone_reverse:
                getattr(self.instance, '_clone_copy_reverse_m2m', self._clone_copy_reverse_m2m)(duplicate, exclude=exclude)
...
class CloneableMixin(models.Model):
    ...
    If commit=True (default) all m2m relations will be cloned and saved, too. This
    includes reverse m2m relations, except if you pass m2m_clone_reverse=False. Even
    intermediate m2m relations (through model) will be cloned, when the intermediate
    model itself is cloneable (has a clone method, does not need to use CloneableMixin,
    but needs to allow at least passing attrs and doing an a automated save like
    commit=True).

    If you don't want the object to be saved directly (commit=False), the clone will
    get an additional clone_m2m method, so you can handle m2m relations outside of
    clone() (see Django forms save_m2m()). This method will save reverse m2m
    relations, if m2m_clone_reverse was True (default). Besides you may overwrite
    this behaviour by passing clone_reverse=True/False.
    ...
    def _clone_attrs(self, duplicate, attrs, exclude=None):
        return self._clone_helper._clone_attrs(duplicate, attrs, exclude=exclude)

    def _clone_copy_m2m(self, duplicate, exclude=None):
        return self._clone_helper._clone_copy_m2m(duplicate, exclude=exclude)

    def _clone_copy_reverse_m2m(self, duplicate, exclude=None):
        return self._clone_helper._clone_copy_reverse_m2m(duplicate, exclude=exclude)
